...
from gdpc.vector_tools import X, Y, Z, XZ, addY, dropY, loop2D, loop3D, perpendicular, toAxisVector2D, Box
...
class Fitness:
    ...
    def check_floating(self, current_building):
        ...
        for base in loop2D((x_pos, z_pos), (x_pos + max_x, z_pos + max_z)):
            ...
            for block, pos in self.map:
                if base == pos and \
                    any(liquid_block in str(block) for liquid_block in liquid_blocks):
                    counter += 1
    ...
    def check_diversity(self):
        unique_buildings = set(self.building_locations[0][:len(self.building_locations)])
